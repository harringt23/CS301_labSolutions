...
public class StarAnimation extends Animation {
    ...
    public void progressChange(int newProgress) {
        //cp4
        // determine the current new progress
        newProgress = newProgress * 10 + 100;

        // determine the difference between the current number of stars and the new progress
       int difference = newProgress - field.size();
        if (difference < 0) for (int i = 0; i < -difference; i++) removeStar();
        else for (int i = 0; i < difference; i++) addStar();
